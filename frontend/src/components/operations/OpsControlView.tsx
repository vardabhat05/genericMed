...
export const OpsControlView: React.FC<OpsControlViewProps> = ({ onExceptionsChange }) => {
  ...
  // Load live exceptions and mappings from backend on mount
  React.useEffect(() => {
    apiClient.operations.getExceptions().then(res => {
      if (res?.data && res.data.length > 0) {
        setExceptions(res.data);
        if (onExceptionsChange) {
          onExceptionsChange(res.data.filter(e => e.status !== 'resolved').length);
        }
      }
    }).catch(() => {});

    apiClient.operations.getMappings().then(res => {
      if (res?.data && res.data.length > 0) {
  ...
  const handleResolveException = async (id: string) => {
    ...
    try {
      await apiClient.operations.resolveException(id, 'Resolved via Ops Tower Dashboard');
    } catch {
      // Local state already updated
    }

    setBannerNotice(`Exception ${id} resolved successfully. Clinical audit trail logged.`);
    setTimeout(() => setBannerNotice(null), 3500);
  };

  const handleMappingDecision = async (id: string, decision: 'approved' | 'rejected') => {
    setMappings(mappings.map(m => m.id === id ? { ...m, status: decision } : m));

    try {
      await apiClient.operations.updateMappingDecision(id, decision);
    } catch {
      // Local state already updated
    }
  ...
  const handleAddNewSalt = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      await apiClient.operations.addCanonicalSalt({
        rawSearchTerm: newSaltName,
        suggestedCanonicalSalt: newEntry.suggestedCanonicalSalt,
