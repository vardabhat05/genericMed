...
export const PharmacyNodeView: React.FC = () => {
  ...
  // Load orders and inventory from backend on mount
  React.useEffect(() => {
    apiClient.orders.getAll().then(res => {
      if (res?.data && res.data.length > 0) {
        setOrders(res.data);
        if (!res.data.some(o => o.id === selectedOrderId)) {
          setSelectedOrderId(res.data[0].id);
        }
      }
    }).catch(() => {});

    apiClient.pharmacy.getInventory().then(res => {
      if (res?.data && res.data.length > 0) {
  ...
  React.useEffect(() => {
    const unsub = apiClient.telemetry.subscribeStream((tick) => {
      if (tick?.temperatureC !== undefined) {
        setTelemetry(prev => ({
          ...prev,
          temperatureC: tick.temperatureC,
          targetRange: tick.targetRange || prev.targetRange,
          status: tick.status || prev.status,
          humidityPct: tick.humidityPct || prev.humidityPct,
        }));
      }
    });
  ...
  const handleStockInScan = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!barcodeInput.trim()) return;

    const code = barcodeInput.trim();
    try {
      await apiClient.pharmacy.scanBarcode(code, 50);
    } catch {
    ...
    const matchedIndex = stockList.findIndex(
      s => s.sku.toLowerCase() === code.toLowerCase() ||
           s.batchNumber.toLowerCase().includes(code.toLowerCase())
    );
    ...
    if (matchedIndex !== -1) {
      ...
      setScanMessage(`Barcode ${code} not mapped to active formulary.`);
      setTimeout(() => setScanMessage(null), 3000);
    }
  };

  // Dispensing verification steps
  const toggleItemVerification = async (itemIdx: number) => {
    if (!selectedOrder) return;
    const targetVerified = !selectedOrder.items[itemIdx].verified;
    const updatedOrders = orders.map(ord => {
      if (ord.id === selectedOrder.id) {
        const updatedItems = [...ord.items];
        updatedItems[itemIdx].verified = targetVerified;
    ...
    try {
      await apiClient.orders.verifyItem(selectedOrder.id, itemIdx, targetVerified);
    } catch {
      // Local state already updated
    }
  };

  const handleSignOffPharmacist = async () => {
    if (!selectedOrder) return;
    const updatedOrders = orders.map(ord => {
      if (ord.id === selectedOrder.id) {
        return {
    ...
    try {
      await apiClient.orders.pharmacistSignoff(selectedOrder.id, 'Dr. Arthur Pendelton, R.Ph #49021');
    } catch {
      // Local state already updated
    }
  };
  ...
  const handleCourierHandover = async () => {
    ...
    try {
      await apiClient.orders.courierHandover(selectedOrder.id, selectedOrder.courierPin);
    } catch {
      // Local state already updated
    }
  };
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <div className="bg-slate-800/60 border border-slate-700/70 p-3.5 rounded-xl">
            <div className="flex items-center justify-between text-slate-400 text-xs font-mono">
              ...
              <Thermometer className={`w-4 h-4 ${telemetry.status === 'excursion' ? 'text-rose-400' : 'text-emerald-400'}`} />
            </div>
            <p className={`text-xl font-bold font-mono mt-1 ${telemetry.status === 'excursion' ? 'text-rose-400 animate-pulse' : 'text-emerald-400'}`}>
              {typeof telemetry.temperatureC === 'number' ? telemetry.temperatureC.toFixed(1) : telemetry.temperatureC}°C
            </p>
            <span className="text-[10px] text-slate-400">Target {telemetry.targetRange} ({telemetry.status})</span>
          </div>
