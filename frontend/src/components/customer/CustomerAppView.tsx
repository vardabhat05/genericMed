...
export const CustomerAppView: React.FC<CustomerAppViewProps> = ({ onOrderCreated }) => {
  ...
  const [medicinesList, setMedicinesList] = useState<MedicineGeneric[]>(MEDICINES_DATA);
  ...
  // Load live medicines from backend
  React.useEffect(() => {
    apiClient.medicines.getAll().then(res => {
  ...
  const handleSimulateRxUpload = async () => {
    setIsUploadingRx(true);
    try {
      const res = await apiClient.prescriptions.extractOcr({
        filename: 'dr_vance_rx_metformin_atorva.jpg',
        mimeType: 'image/jpeg',
      });
      setRxUploaded(true);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setPaymentDone(true);
      if (onOrderCreated) {
        onOrderCreated(orderData.orderNumber || orderData.id);
      }
    } catch {
      const fallbackOrder = {
        id: `ord-${Date.now()}`,
      ...
      setPaymentDone(true);
      if (onOrderCreated) {
        onOrderCreated(fallbackOrder.orderNumber);
      }
    } finally {
              ...
              {!paymentDone ? (
                <>
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white font-extrabold text-sm rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-emerald-900/20 transition"
                  >
                    {isCheckingOut ? (
                    ...
                    <span>{isCheckingOut ? 'Authorizing Escrow...' : `Authorize & Pay $${finalTotal.toFixed(2)}`}</span>
                  </button>
                </>
              ) : (
                ...
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 text-center space-y-4 my-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-600 mt-1">
                      Order #{completedOrder?.orderNumber || 'ORD-98241'} routed to Apollo Pharmacy Store Node #4.
                    </p>
                  </div>
                  ...
                  <div className="bg-white p-3 rounded-xl border border-emerald-200 text-left font-mono text-[11px] space-y-1">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-slate-900">
                        {completedOrder?.courierName || 'Jason K.'} (PIN: {completedOrder?.courierPin || '8492'})
                      </span>
                    </div>
                  </div>
